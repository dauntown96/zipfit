...
> **마지막 업데이트**: 2026-07-05 (1차 배치 5건 프론트엔드 노출 작업 완료 — DB만)
...
- **프론트엔드에 나머지 신규 데이터 노출**: scoring_criteria(가점표), 이번 배치의 "🔓자격완화 추가모집" 뱃지(다운님과 UI 설계 논의 후 진행)
- **1차 배치 5건 실사용 확인 필요**: 양산사송/군포송정/군포대야미(precise_address만) + 경기남부 신혼신생아Ⅰ·Ⅱ전세형(housing_units 342/169건) 5개 공고를 다운님이 실제 사이트에서 열어 지도·세대정보·사진 썸네일이 정상 노출되는지 확인 필요(Playwright는 카카오 API 접근 불가로 검증 못함)
...
- **announcement_extras 파일 목록이 완전하지 않을 수 있음**: Drive 폴더 페이지네이션 한계로 각 폴더의 1페이지(최대 10~14개) 파일만 확인함 — 특히 경기남부 신혼신생아Ⅰ·Ⅱ(각 300+개 세대가 있는 대규모 공고)는 건물별 팜플렛/평면도가 폴더에 더 있을 수 있음, 추가 확인 시 폴더 재조회 필요
