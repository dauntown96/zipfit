...
| 날짜 | 내용 |
|---|---|
| 2026-09-25 | **H1 — 목록 RPC `get_announcements_deduped`가 anon statement timeout(3s)으로 HTTP 500 — `cancel_keys`를 `MATERIALIZED`로**(🔴 **함수 1(`CREATE OR REPLACE` — 마이그레이션 `h1_cancel_keys_materialized`, 라이브 md5 `72da7fa5`일 때만 한 조각 치환하도록 가드 · 반환 타입·ACL 불변) · DDL 0 · 데이터 쓰기 0 · EF 0 · 코드 0(`index.html`·`sw.js` 불변) · `supabase/rpc/` 덤프·README · Drive 0 · Notion ⑩ 1절** · `[zipfit-backup]` 접촉 0). ① **원인** — B30(10:15Z)이 `has_cancel_notice`를 `title_key`+`apply_end` EXISTS로 바꾸자 `cancel_keys` CTE가 인라인되어 대표행 906개마다 `base` 3,066행을 다시 훑었다(`loops=906`). anon `statement_timeout` 3s에 전 컬럼 REST 직렬화가 더해져 비로그인 목록이 비었다(Edge 500 10시대 8 · 13시대 5). ② ✅ **의미 불변** — 옛 정의 ↔ `pg_temp` 새 정의 전 칸 `EXCEPT ALL` 양방향 0 · 906 = 906 · 인자 3종 차 0(13:25Z). ③ ✅ **속도** — anon · 3s · 전 컬럼 1,842ms → **177ms**(전 행 md5 `a9799b7c…` 같음) · `EXPLAIN ANALYZE` 1,734 → **155ms** · 같은 꼴 점검: `get_announcement_price_summary`(906 ID) 794ms · `get_announcement_group_ids` 28ms · `get_announcement_blocks` 62ms. ④ ✅ **운영** — 공개 REST(anon 키) 200 · 906행 · 적용 뒤 Edge 500 = 0(마지막 500 13:21:49Z). md5 `72da7fa5…` → `b6b88a8a…`. 🔴 **B30 검증은 postgres 대조와 RPC 응답을 막은 화면 대역뿐이라 시간 초과가 드러나지 않았다** — 재발 방지 원칙 초안은 claude.ai 회신 ②. |
