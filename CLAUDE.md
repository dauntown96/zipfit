...
| 2026-09-25 | **B33 — 충북 9/17 매입임대 3공고(`…020774` 전세형 · `…020776` 신혼Ⅰ · `…020778` 청년) 해설·요약문 정리 + `…020206` 「성별분리」 1행 반영**(🔴 **정책 UPDATE 50 · INSERT 15 · 자격 UPDATE 12(`verification_requirements` · 0778 `manual_check_note` → NULL 3) · DDL 0 · 함수 0 · EF 0 · 코드 0 · Drive 0 · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 reverify v4.7 · analysis v9.0을 반영 전에 다시 불러왔다. 공고별 트랜잭션에 가드(행마다 BEFORE md5 · AFTER md5 · 행 수 · NFC)를 걸었다. ① ✅ **확인서 조정** — p.1 머리 □ 마이홈센터·• (신청유의)는 세 공고 모두 「공급개요」로, p.14 맺음말은 세 공고 모두 싣지 않았다. p.2 표 아래 * 두 줄은 「임대조건」 행으로 옮겼다(형제와 같게). 0778 신규 N5 「MyMy서비스 이용 안내」·N7 「공급대상주택 목록」은 뺐다. 온라인 서류제출 방법 신규 행은 걷는 요약이 인용하던 문장만 남겼다(0776 1줄 · 0778 3줄). 0778에서 뺀 N5가 담던 MyMy 두 문장(세대원 전원 동의 · O 표시 서류)은 서류제출 행에 붙였다(0774와 같은 꼴). ② ✅ **원문 날짜 오류 ⚠️ 신규** — 0778 p.1 모집일정 「자격검증 10.09(목)」, 2026-10-09는 금요일(쪽 이미지로 확인). 0774·0776은 「10.09(금)」이라 해당 없음. ③ ✅ **행 수** 정책 27 · 28 · 27(착수 23 · 23 · 21, 신규 4 · 5 · 6) · 자격 12행 JSON = AFTER(jsonb 텍스트 md5 12/12) · `…020206` `9949cc26`는 형제 `…020819` `5d70ed6f`와 category·md5 `0bf03fd2…` 바이트 동일 → 카드에서 한 줄. ④ ✅ 앵커 재검: 교체·신규 전 줄이 에이전트 좌표 검사 통과 줄의 부분집합이다. 새로 쓴 것은 0778 ⚠️ 행과 (접속방법) 첫 문장뿐이고, 둘 다 원문 대조 통과 · 불변식 1·6·7·9·10 0 · 형제 스냅샷 · 광주전남 `…020773`·`…020777`·`…020784` 정책 md5 불변 |
