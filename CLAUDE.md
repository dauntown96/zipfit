...
> **마지막 업데이트**: 2026-07-06 (공고 수집 주기 2회→4회/일 확대 + 실행결과 영구 로그 테이블 추가)

## 🔜 다음 세션 작업 예정
- **🟡 MYHOME upsert 배치 첫 청크에서 반복되는 conflict 에러**: `collection_run_log`/응답 errors에 매 실행마다 `"MYHOME upsert[0]: ON CONFLICT DO UPDATE command cannot affect row a second time"` 1건씩 발생 확인(v11/v12 테스트 실행 2회 모두 재현). 같은 upsert 배치(50건) 안에 `announcement_id`가 중복된 MYHOME row가 섞여 있는 것으로 추정 — 기능 자체는 정상 동작(나머지 49건은 반영됨)하지만 원인 규명 및 배치 내 중복제거 로직 추가 필요
