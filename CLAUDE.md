...
| 2026-09-25 | **B36 — 접수 9/29 활성 공고 잔여 정리(1차: 항목 4·5·6 반영 · 항목 1~3 분류 후 멈춤) + 원칙 29 신설**(🔴 **정책 INSERT 1(양산 `…020754` `ce9e8dd2`) · UPDATE 0 · DELETE 0 · 세대 0 · 자격 0 · DDL 0 · 함수 0 · EF 0 · 코드 0(`index.html`·`sw.js` 불변) · `CLAUDE.md` 원칙 29 · Drive 0 · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 reverify v4.7 · analysis v9.0을 착수 전에 다시 불러왔다. ① ✅ **양산 ⚠️ 행 되살림**(항목 4 — B34 확인서 8 정정) — category 「⚠️ 편의시설 문의처 표기 주의」 · 셋째 줄 원문은 PDF p.20(인쇄 번호 「- 20 -」, 줄바꿈 이음) · hwpx 문단과 바이트 동일 · NFC · md5 `d3e8dac6…` · 그룹 안 같은 문장 0행이던 것 확인 뒤 INSERT. 정책 9 → **10** · 기존 9행 md5 불변 · 불변식 1·2·6·7·9·10 = 0/불변 · 형제 `…020407` 정책 md5 불변. ② ✅ **원칙 29** — RPC·DB 함수 검증은 부르는 역할(anon) · 그 역할의 `statement_timeout` · 전 컬럼으로 잰다 / 한도의 1/3 초과 시 `EXPLAIN ANALYZE` / 적용 뒤 공개 REST 200·행 수. ⚠️ H1 회신 ② 1 초안 원문은 저장소·PR [#220](https://github.com/dauntown96/zipfit/pull/220)에 없어 **요청서가 요약한 네 요소로 썼다**(claude.ai 대조 요청). ③ ✅ **`source_file` 실재 측정**(항목 6 · 고치지 않음) — 「(YYYYMMDD).pdf」 꼴 15종(세대 130 · 정책 40)의 Drive 제목 정확 일치: **있음 2 · 없음 13**(NFC·NFD 둘 다). 보조 `contains` 검색으로는 영월주천 1종이 더 잡혀(정확 일치 검색이 놓침) 실재 3 · 나머지 12종 중 9종은 다른 파일명의 후보가 있고 3종은 후보가 없다. ④ ⏸ **항목 1~3 멈춤** — `extra_note` 5칸(음성삼성 51 `1898d2f9` · 창녕 36 `aa85d6d3` · 46 `74442885` · 양산 46㎡ `b5601865` + 형제 음성금왕3-1 59 `960ffebd`) · 원주 `27c304e0` ④ 장애정도 칸 밀림(hwpx 셀 + PDF p.11 좌표로 확정) · 군산 `2073b9de` · 장성 `8b18b89d` · 담양 `7a99fb39`(세 겹 재판정: 모순 유지 → 보성 꼴 교체안). 확인서 대기. |
