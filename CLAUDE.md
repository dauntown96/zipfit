...
> **마지막 업데이트**: 2026-07-06 (2차 배치 5건 스키마·백필·검증·사진 반영 완료 — DB만)
...
- **프론트엔드에 나머지 신규 데이터 노출**: scoring_criteria(가점표), 완화모집/선착순 배지(다운님과 UI 설계 논의 후 진행)
...
- **🔴 eligibility_criteria "소득·자산 무관" 표현 방식 미정**: 충북음성 든든전세(분양전환형)는 QnA상 소득·자산 요건이 아예 없는 유형인데, `eligibility_criteria` 스키마에 "무관"과 "미분석(NULL)"을 구분할 필드가 없어 현재 모든 수치 필드가 NULL인 row로만 등록됨(겉보기엔 미분석과 동일) — 필드 추가 여부 claude.ai/다운님 결정 필요
