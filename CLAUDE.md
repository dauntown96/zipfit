...
> **마지막 업데이트**: 2026-07-06 (김포마송 블록 분리 반영 + special_notes JSONB 신설)

## 🔜 다음 세션 작업 예정
- **🔴 김포마송 B-3BL/B-5BL, `get_announcements_deduped()`에서 카드 1개로 병합되는 구조적 문제 발견**: 블록별 매핑은 확정함(근거: LH 행 area_min/max=21.88~46.79가 마송6 유닛타입 구성과 정확히 일치해 LH 2건(id 10398/10400)=마송6=B-3BL 확정, MYHOME 199세대(18729/18734)=마송6/B-3BL, 277세대(18728/18733)=마송5/B-5BL로 확정 — 공고문의 "예비자(283)"/"(193)" 표와도 방향 일치). `precise_address`(B-3BL=율마로450번길51 / B-5BL=율마로438번길51)와 `announcement_extras`(B-3BL 11건/B-5BL 10건, 파일명 라벨 기준)를 각 raw id별로 정확히 분리 백필 완료. **하지만 `get_announcements_deduped()`의 dedup_key가 제목만으로 그룹핑해서 두 블록이 동일 제목(공통 표제)이라 결국 대표행 1개(현재 id=10398, B-3BL 쪽)로만 병합 노출됨** — B-5BL(마송5) 쪽 정보는 DB엔 정확히 있지만 RPC/프론트엔드에는 절대 노출되지 않는 상태. 두 블록을 별도 카드로 분리 노출하려면 dedup_key 로직 자체를 손봐야 하는데(예: 본문에서 블록 마커를 추출해 dedup_key에 포함), 이는 다른 452건 전체의 병합 로직에 영향을 줄 수 있는 변경이라 임의로 진행하지 않음 — 다운님 확인 후 별도 작업으로 진행 필요
- **🔴 부천원종 "지자체검증 필요" 필드 신설 여부 확인 필요**: 부천원종 C1블록(일자리연계형 지원주택, id 10399)은 직업요건(가-1 창업인·지역전략산업종사자/가-2/나 산업단지·중소기업근로자)이 LH접수 후 지자체(부천시) 검증을 거치는 특수 신청구조인데, 이 "지자체검증 필요" 상태를 표현할 전용 컬�럼이 `announcements`에 없음 — 기존 `relaxation_detail`류 필드를 재사용할지, 신규 컬럼을 만들지 판단이 필요해 이번 세션엔 보류(임의로 컬럼 신설 안 함). eligibility_criteria에 직업요건 가-1/가-2/나 상세 텍스트를 넣을 자유텍스트 필드도 현재 스키마엔 없어(income_pct 등 숫자형 컬럼뿐) 함께 결정 필요
- **🟡 인천계양A3/인천가정2A2 eligibility_criteria(가구원수·출산자녀 가산, 지역우선순위) 및 하남풍산1단지(청약통장 전환시점 규칙, 순위별 접수일 규칙) 미반영**: 위와 같은 이유로 `eligibility_criteria` 테이블에 이 정보를 담을 자유텍스트 컬럼이 없어 이번 세션엔 반영 못함 — 스키마 확장 여부 다운님 확인 필요
