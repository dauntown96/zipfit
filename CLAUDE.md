...
> **마지막 업데이트**: 2026-07-06 (김포마송 2블록 아코디언 분리 표시 — 대표카드 유지 + 상세 아코디언 안에서 블록 분리)

## 🔜 다음 세션 작업 예정
- **🟢 (해결됨) 김포마송 2블록 표시**: `get_announcement_blocks(p_announcement_id)` RPC 신규 추가 — 같은 dedup_key(제목)를 공유하는 원본 행들 중 `precise_address`/`total_units`가 서로 다른 그룹을 블록 단위로 반환. 2개 이상 블록이 있을 때만 상세 아코디언에 "🏘️ 이 공고에 포함된 블록" 섹션이 표시되고(김포마송: 마송5/B-5BL 277세대·마송6/B-3BL 199세대 2개 블록 확인), 블록이 1개뿐인 일반 공고는 기존과 동일하게 동작(회귀 없음, 특정 공고 하드코딩 없는 범용 로직). 대표 카드(`get_announcements_deduped()`의 병합)는 그대로 유지 — dedup_key 로직 자체는 건드리지 않아 나머지 461건에 영향 없음
