...
| 날짜 | 내용 |
|---|---|
| 2026-09-26 | **B39 — 9/17 매입임대 템플릿 6공고(충북 `…0774`·`…0776`·`…0778` · 광주전남 `…0773`·`…0777`·`…0784`)에 어느 행에도 없던 원문 블록 4종 신규 + pp.16–19·QnA 측정 + `…0778` 순위 이름 멈춤**(🔴 **정책 INSERT 24(공고마다 4) · UPDATE 0 · DELETE 0 · 세대 0 · 자격 0 · DDL 0 · 함수 0 · EF 0 · 코드 0(`index.html`·`sw.js` 불변) · Drive 0 · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 reverify v4.8 · analysis v9.1을 착수 전에 다시 불러왔다. ① ✅ **신규 4블록**(항목 1) — 「금융자산 조회 안내」(p.20 · 「…제출하지 아니할 경우 신청이 불가합니다.」 + 동의서 표 5칸, hwpx 셀로 「서명」 2행 병합 확인) · 「주택 소유여부 확인 방법 및 판정기준(주택공급에 관한 규칙 제53조)」(p.15, 35줄) · 「온라인(PC, 모바일) 서류제출 방법」(청년 p.9 · 신혼 p.11) · 「MyMy서비스 이용 안내」(청년 p.6 · 신혼 p.7). 원문 바이트는 광주전남 hwpx(`lineBreak`는 줄바꿈, `fwSpace`는 PDF에서 공백이 없어 빼냄)에서 떴고, 충북 3건은 공고문 hwpx가 Drive에 없어 **자기 PDF에 전 줄(104줄)이 정규화 일치하는지** 확인한 뒤 같은 바이트를 썼다. 형제 여섯의 같은 블록 md5는 금융자산 `e7bb7052…`·판정기준 `3b774043…`가 여섯 모두 같다. 온라인·MyMy는 공고마다 이미 있던 문장을 빼서 갈린다(0774·0776 파일 형식 1줄 · 0778 3줄 · MyMy 0774·0778 2줄 · 0776 4줄). 신혼Ⅱ(전세형) 두 건(`…0774`·`…0784`)은 원문 마지막 줄에 닫는 괄호가 없어 그대로 두었다. 정책 행 25·27·28·27·27·26 → **29·31·32·31·31·30**. ② ⏸ **교체 후보 1**(멈춤 규칙) — `…0778` `324608dd` 첫 줄이 원문 (접속방법) 두 문장 중 첫 문장만 싣고 있어 그 조각은 넣지 않았다. ③ ✅ **측정**(항목 2) — pp.16–19 「소득·자산 산정방법(상세)」 표 8행(신혼Ⅱ 7행) · 「소득항목 설명 및 소득자료 출처」 표 30행(29행), 결과어(신청 불가·탈락·취소 등) 0 · 「제외」(산정 제외)만. QnA 38·39·40문항 중 행에 반영된 것 2·3·3, 반영 안 된 문항 중 결과어 포함 4·7·7. ④ ⏸ **`…0778` 순위 이름**(항목 3 · 멈춤) — `c999a026` · `c8221e1b` 괄호 속 사실은 같은 공고 정책 `80852c4b`(입주순위)·`dd5e7e6c`(소득·자산 기준)에 모두 있다. 교체안(`…0773`과 같은 「2순위」·「3순위」)만 회신에. ⑤ ✅ **검증** — 여섯 그룹의 블록·그룹 ID·세대·자격 행 수와 md5·기존 정책 행 md5 착수 스냅샷과 같음 · 불변식 1·9 = 0 · NFC 위반 0 · 금융자산 「신청이 불가합니다」 행 여섯 모두 있음 · 🅑 분석률 분자 불변(분석 산물만 고친 회차) |
| 2026-09-26 | **B40 — 군산나운4 9/18 회차 `…020801` v9.1 정리(세대 `extra_note` 2 · `relaxation_detail` 1) + B35 보충 행 9/18 원문 대조 + 7월 MYHOME `20722_1` 세대 2행 분류 후 멈춤**(🔴 **세대 `extra_note` UPDATE 2 · `announcements.relaxation_detail` UPDATE 1 · INSERT 0 · DELETE 0 · DDL 0 · 함수 0 · EF 0 · 코드 0(`index.html`·`sw.js` 불변) · Drive 0 · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 analysis v9.1 · reverify v4.8을 착수 전에 다시 불러왔다. ① ✅ **세대 2행**(항목 1 · 조건부 사전 승인) — `7064ab26` `67dbbece…` → `e0c530c8…` · `32bfdd1a` `be5d043c…` → `2b8a6d49…`(지시서 AFTER 전문과 로컬 md5 일치 확인 뒤 BEFORE 가드로 1행씩). 우리 연결어·「(다른 조건 칸)」·`원문: ※ …`·⚠️ 재료 없음 문장을 걷고 「(26형 전체)」는 유지. ② ✅ **`relaxation_detail`**(항목 2) — 마지막 줄(재계약 3곳 모순 문장)만 걷어 `2e38b7c9…` → `3a05ff98…`(= `…020822`와 같은 값). ③ ✅ **보충 행 5개 대조**(항목 3 · 측정) — 13문장 전부 9/18 hwpx 문단과 바이트 일치 · PDF p.12·p.13에 있음(한 문장은 표 행 레이블 「대상자」·「결정및」 끼어듦) → 그대로 둠. ④ ⏸ **7월 `20722_1` 세대 2행 멈춤**(항목 4) — 7월 원문은 Drive에 PDF만 있다(hwpx 없음). p.2 표 값을 단어 좌표로 칸에 맞춰 AFTER안 2개를 만들고 값마다 앵커를 통과시켰다(9/18과 다른 값은 현재 공가 호수 **449**, 9/18은 447). 반영하지 않음. ⑤ ✅ **검증** — 그룹 7 ID의 세대(대상 2행 밖)·자격·정책·공고 md5가 착수 스냅샷과 같다 · 블록 `1096a54c…`·그룹 `be65f77f…` 불변 · 불변식 1·5·6 = 0 · 9·10 대상 없음(extras 0). 첫 실행은 불변식 7 가드가 그룹 전체의 7월 행(「가군/나군」 표기)에 걸려 롤백됐고, 가드를 대상 ID로 좁혀 다시 돌렸다 · 🅑 분석률 분자 불변(분석 산물만 고친 회차) |
