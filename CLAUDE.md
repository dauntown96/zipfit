...
| 날짜 | 내용 |
|---|---|
| 2026-09-25 | **B26 — 🔴 P1: 열린 지난 회차를 목록에 되살렸다(군산나운4 `…020801` 접수 9/29) · 이미지 썸네일 실패를 「미리보기 없음 · 눌러서 보기」와 `/preview` 모달로 · 설정 탭 총자산 안내**(🔴 **DB 함수 `get_announcements_deduped()` `CREATE OR REPLACE`(마이그레이션 `b26_p1_open_past_round_split` → `…_tiebreak` → `…_tiebreak_asc` → `…_tiebreak_desc`, 최종 md5 `50a0357c…` → `819f1443…` = 저장소 덤프, 반환 타입·ACL 불변) · 테이블 쓰기 0 · DDL 0 · EF 배포 0 · 코드 변경 `index.html`(`extrasThumbFailed`·`refreshExtrasCounts`·모달 `/preview` 폴백 · `set_assets` 안내) + `sw.js` `CACHE_NAME` v138 → **v139** + `supabase/rpc/get_announcements_deduped.sql`·README + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 1파일(`fetch-attachment` → `[임시] 2015122300020638`, 태백 8/31 공고문 PDF — 확인용) · Notion 쓰기 ⑩** · `[zipfit-backup]` 접촉 0). ① ✅ **P1** — 대표와 회차 날짜·`apply_end`가 둘 다 다르고 LH `status`가 `공고중`·`접수중`인 회차만 `dedup_key`에 `#회차날짜` 꼬리(같은 회차 MYHOME 짝 포함). 적용 직전 재측정(03:09Z) 걸린 것은 군산 한 그룹. 「종전 정의 + 같은 꼬리키」를 같은 쿼리에서 계산해 대조(03:17:52Z): 905 → 906, 늘어난 것은 `…020801` 하나, 나머지 905행 15필드 동일(`…020822` 포함), 취소공고 배지 2건 불변. ② 🔴 **`best_schedule` 동률 꼬리키 `id DESC`** — `created_at`까지 같은 형제 행에서 뽑히는 값이 실행 계획에 따라 갈렸다(꼬리키 없는 종전 식이 03:15Z엔 `id ASC`와, 03:18Z엔 `id DESC`와 같았다 — 고정된 종전 값 없음). `best_location`과 같은 방향이라 표지 주소와 건물명이 같은 블록 행에서 온다(146그룹 중 142). ③ ✅ **함께 확인** — `…020801` 1층 요약 `current`·월 74,730~116,560원(0801 세대만, 20722_1 오류값 없음) · `…020822` 요약은 여전히 `past`에 391,400 혼입(범위 밖) · 0822 카드 세대는 「지난 회차 보기」에 접히고 정책은 ⚠️ `MIXED_POLICY` · 🔴 태백 `…020638`(8/31, 원문 접수 9/14~10/14)은 **`[취소공고]` `…020645`(같은 8/31·10/14)가 있는 취소된 회차**라 P1이 떼지 않는 것이 맞다. ④ ✅ **썸네일 폴백** — 배포 코드 전·후 대역(B25 표본 실패 `1QGd7n`·`1qfRLX` · 정상 `1tsqX_` + 팸플릿·ID 없는 행): 실패 타일 「📄 미리보기 없음 · 눌러서 보기」·모달 `/preview` · 정상 타일·모달 불변 · 썸네일은 떴는데 모달 이미지만 실패하면 `uc`→`w1600`→`/preview` · 제목 「(4건) · 3건 미리보기 없음 · 1건 불러오기 실패」 · `pageerror` 0. ⑤ ✅ **설정 탭** 안내 줄 · 저장 페이로드 전 = 후. ⑥ **미확인**: 🔴 배포본 육안(군산 카드 두 장 · 썸네일 폴백) — claude.ai Playwright 몫 |
