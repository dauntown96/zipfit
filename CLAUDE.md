...
| 날짜 | 내용 |
|---|---|
| 2026-09-25 | **B28 — 사진·평면도를 세대 행 묶음에 잇는 컬럼 신설 · 백필 1,033행 · 건물·주택형 머리의 「📷 N」 칩**(🔴 **DDL 1(마이그레이션 `b28_extras_unit_key` — `announcement_extras`에 nullable `unit_key`·`unit_key_source` + CHECK 2 · 코멘트 수정 `…_comment_composite`) · UPDATE 2회(한 트랜잭션씩) · 함수 0 · EF 0 · 코드 변경 `index.html`(칩 자리·첨부 로더가 세대 머리 키를 기다려 가름) + `sw.js` `CACHE_NAME` v139 → **v140** + `supabase/rpc/README.md` DDL 절 · Drive 0 · Notion ⑩** · `[zipfit-backup]` 접촉 0 — 새 컬럼은 다음 백업에 나타난다). ① ✅ **키 규약** — 매입임대 = 건물 머리 키(`groupHousingUnits`와 같은 식) · 단지형 = `건물::주택형`(아산 `…020726` 「46형」이 두 건물에 걸려 이름만으로는 1:1이 안 선다) · `unit_key_source` 3종(`string_match`·`supply_list`·`image_link`). ② ✅ **백필** — 매입임대 815(B25 규칙 그대로 · 모호 15·미적중 210 제외) + 단지형 218(주택형만으로 223 → 두 건물에 걸린 5행 NULL) · B27 죽은 210행 0 · 앞 8컬럼 md5 `92d5f573…`·`id‖file_url` `435ed0e3…` 불변 · 키가 같은 ID 세대 행에 안 닿는 행 0. ③ ✅ **화면** — 머리에 닿는 행만 칩, 나머지는 카드 갤러리(「N건은 📷 칩에」 한 줄) · 머리가 없으면 전부 갤러리. 대역(DB 봉인 canonical md5 · 360px): 서울 청년 `…020759` 칩 109 합 228 + 갤러리 3 = 231 · 아산 `…020726` 9 + 22 = 31 · 경북서부 `…020709` 블록 5개 11 + 11 = 22 · 키별 N 불일치 0 · 칩 펼치기 전 썸네일 요청 0 · 머리 넘침 0 · 키를 걷으면 `origin/main`과 갤러리 같음 · Drive 404면 칩 모달도 `/preview` · `pageerror` 0. ④ **미확인**: 🔴 배포본 육안 — claude.ai Playwright 몫 |
