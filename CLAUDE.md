...
| 2026-09-25 | **B29 — 7/15 회차 17공고의 끊긴 이미지 행 210행 정리: 백업 → 삭제**(🔴 **DB 삭제 210행(한 트랜잭션·가드 5) · DDL 0 · 함수 0 · EF 0 · 코드 변경 0(`supabase/backups/extras_dead_7-15_20260925.json` 추가) · Drive 쓰기 0 · Notion ⑩ 1줄** · `[zipfit-backup]` 접촉 0). ① ✅ **재측정**(04:29~04:36Z, 렌더러와 같은 `thumbnail…w300`, 207개 · 52개씩 · 끊긴 요청 12개 재시도): 전부 로그인 페이지 · 살아난 ID 0 · 대상 210 그대로. B28 연결 컬럼은 이 행들에서 전부 NULL. ② ✅ **백업 먼저** — 전 컬럼 10 · 210행 · seal md5 `67a0851a…` = DB · 원격 blob 재확인 · PR #213 → `93cab90` 병합 뒤 삭제 · 복원 `json_populate_recordset` 한 줄(표본 2행 전 칸 일치). ③ ✅ **삭제** — 가드: 파일 판정 전부 로그인/404 · 대상 210 · 대상 seal = 백업 seal · `file_url` 일치 · 남은 행 md5 = 「대상 제외」 md5. 2,016 → **1,806** · `id‖file_url` `5723b1c0…` · 경북서부 `21266_x` 22 불변 · 연결 1,033 불변. ④ 🔵 **활성 카드** — 원주 `…020783`(블록 6 중 `20759_3`·`20759_6`) 끊긴 타일 5 → 0 · 경북서부 `…020709`는 블록이 `21266_x`뿐이라 전·후 같다(끊긴 행이 애초에 닿지 않았다). ⑤ 산물 없는 LH ID 8개의 extras 35행 중 21행이 이번에 빠지고 14행 남음(`…0243` 1 · `…0261` 9 · `…0326` 1 · `…0699` 3) |
