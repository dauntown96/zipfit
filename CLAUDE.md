...
| 날짜 | 내용 |
|---|---|
| 2026-09-25 | **B27 — 7월 이미지 207개(210행·53공고) 재연결 조사. 적용 0행: 되이을 「현재 파일」이 Drive 어디에도 없었다**(🔴 **DB 쓰기 0(SELECT와 `net.http_get` 썸네일 관측만) · DDL 0 · 함수 0 · EF 0 · 코드 변경 0 · Drive 쓰기 0 · Notion 쓰기 ⑩ 1줄** · `[zipfit-backup]` 접촉 0). ① 🔵 **폴더 대응** — 앱 루트 하위 177폴더 전수: 날짜형 90개 중 이 53공고(7/15 회차 17그룹)의 폴더 0. 지시서 표본 `(도산)`·`(대신휴먼시아)`는 **다른 공고**(`20764_1`·`20756_1`)의 폴더이고 그 파일은 이미 거기 이어져 있다. `[임시] <7/15 LH ID>`는 16그룹에 있으나(9/17) 전부 공고문 PDF 1개뿐 · `…020399` 산곡은 폴더 없음. ② 🔵 **Drive 전역** — 단지명 40여 개 제목 검색(9/1 이전) · 일반 파일명 정확 일치 · 7/14~8/1 이미지 첫 쪽 200개: 전부 다른 공고. 옛 ID 표본 7개 커넥터 `not found`. ③ ✅ **분류** ⓐ 0 · ⓑ 0 · ⓒ 8행(`…0399`) · ⓓ 202행 · 적용 0 → `announcement_extras` 2,016행·전 행 md5 `435ed0e3…` 불변(03:48Z). 활성 카드 2장(원주 `20759_3/6/7` · 경북서부 `20826_1/2/4/5/7`) 43개 전부 로그인 페이지(03:46Z, 전=후). ④ 🔵 **다음 회차 사본은 따로 있다** — 경북서부 9/15 `[임시] …020709`(`21266_x`에 연결) · 원주 9/15 `[임시] …020783`(미연결, B29) · 태백장성 9/14 — 다른 회차라 7/15 행을 잇지 않았다. ⑤ **미확인**: 삭제인지 공유 해제인지 |
