...
> **마지막 업데이트**: 2026-07-05 (공고 분석 1차 배치 5건 반영 — 신규 카테고리 3종 스키마 반영, DB만)
...
- **프론트엔드에 나머지 신규 데이터 노출**: scoring_criteria(가점표), 이번 배치의 "🔓자격완화 추가모집" 뱃지(다운님과 UI 설계 논의 후 진행) — housing_units·announcement_policies·eligibility_criteria·announcement_extras는 반영 완료
