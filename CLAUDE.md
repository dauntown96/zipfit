...
> **마지막 업데이트**: 2026-07-06 (정정공고 3건 점검 후속조치 — revision_note/revised_at 신설, 최신순 정렬 버그 수정, sw.js v34)

## 🔜 다음 세션 작업 예정
- **🟡 is_revised 필드가 [정정공고] 제목 접두어와 불일치하는 사례 존재**: LH `PAN_SS`(status) 필드에 "정정" 문자열이 없는데도 제목엔 `[정정공고]` 접두어가 붙은 공고 발견(예: id 10398 김포시, 1152 김포시, 파주시 등) — `is_revised`/`revised_at`이 이런 케이스를 놓쳐서 실제로 정정된 공고인데도 최신순 정렬에서 우선순위를 못 받음. `is_revised` 계산 로직에 제목 접두어 검사도 추가할지 검토 필요(이번 세션은 정렬 로직 자체 수정에 집중, 판단 로직 확장은 범위 밖으로 보류)
