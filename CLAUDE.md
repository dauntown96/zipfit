...
| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **B18 — 활성 9공고의 지난 회차 정책 44행 중 25행을 원문 문장으로 바꾸고(확인서 승인분 그대로 · 308 `e148b059`는 ⚠️ 병기 행으로), 206 QnA 7번을 새 행으로 떼어 넣고, `source_file` 3행을 실파일명으로 고치고, ⚠️ 병기 행 셋(308 `e148b059` · 649 `57b177a7` · 249 `1dd673da`)의 연결 문장 「같은 별지N의 항목 2·3은 다음과 같습니다.」를 위치 표기로 바꿨다**(🔴 **DB 쓰기 `announcement_policies` UPDATE 31행(교체 25 · category만 1(`3a8ecf78` → 「불법전대자 입주자격 제한」) · `source_file`만 3 · 기존 ⚠️ 행 연결 문장 2) · 신규 1행(206 QnA) · DELETE 0 · 총수 1,327 → **1,328** · DDL 0 · RPC·트리거 0 · EF 배포 0 · 코드 변경 0(`index.html`·`sw.js` 미변경 → `CACHE_NAME` 그대로 **v133**) + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 0 · Notion 쓰기 0** · `[zipfit]` 이력만 · `[zipfit-backup]` 접촉 0). ① ✅ **교체 25** — AFTER는 멈춤 회신 첨부의 코드 블록을 스크립트로 떠서 그대로 썼다(옮겨 적지 않음 · 26블록 AFTER md5 전부 첨부 표기와 일치). UPDATE마다 BEFORE md5 가드 · 1행 적중 · AFTER md5 일치를 걸고 네 트랜잭션으로 나눴다(`1894d060`+`3e4f1d41` · `c84ddc74`+신규는 각각 같은 트랜잭션). 같은 문안 행은 DB 안에서 첫 행을 복사했다. ② ✅ **자기 점검** — 원문 10파일(202·208·209·206+QnA·249·308·352·407)을 Drive에서 받아 pdfplumber로 AFTER 전 줄을 정규화 대조했다. 원문 밖으로 남은 것은 평탄화 구분자(세로줄 · `—` · `/` · `→` · `:`)와 ⚠️ 병기 행의 허용 문장·위치 표기뿐이다. 건너뛴 행 **0**. ③ ✅ **category** — `3fb04fa5` → 「기타 제출서류」(202 p.8 「■ 기타 제출서류 (아래 표에 해당하는 자만 제출)」 표 안 문장) · `e148b059` → 649 `57b177a7`와 같은 ⚠️ 이름 · `3a8ecf78` → 649 `07156fee`와 같은 이름(내용 md5 같아 카드에서 한 줄). ④ ✅ **위치 표기 쪽수** — 308 p.34 · 249 p.37 · 649 p.35(649 PDF는 18쪽이나 확약서 쪽 인쇄 번호가 「- 35 -」 — 행 안 기존 표기와 맞춤). ⑤ **검증** — 총수 1,328 · 손대지 않은 1,296행 md5 `be0a0e71…` 전·후 **불변**(형제·그룹 밖 포함) · 44행 = 교체 25 + 유지 19 · 옛 회차 9 ID의 일반명 `source_file` **0** · ⚠️ 병기 행 셋 「다음과 같습니다」 **0** · 허용 문장 ①② 각 1회 이하 · 🅑 **76/41 · 7/1 → 76/41 · 7/1**(05:36:07Z → 05:40:39Z, 분자 불변). ⑥ **하지 않은 것**: 206 개보수 문장 **0**(백로그 「과거 배치 재점검」) · 우선배정 두 행 형제 문안화 **0**(선택 ② 기각) · 「청약통장 요구여부」 나머지 2행 **0** · `[zipfit-backup]` **0**. ⑦ **미확인**: 없음 — 코드 변경이 없어 배포본에서 볼 것이 없다 |
