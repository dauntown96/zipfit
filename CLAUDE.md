...
> **마지막 업데이트**: 2026-07-06 (3차 배치 5건 반영 — 인천계양A3/부천원종/인천가정2A2/김포마송/하남풍산1단지)

## 🔜 다음 세션 작업 예정
- **🔴 김포마송 B-3BL/B-5BL 블록 분리 여부 확인 필요**: DB에는 LH 2건(id 10398 정정/10400)·MYHOME 4건(id 18728/18729/18733/18734, houseSn 2·3 세대수 277/199)이 이미 존재하며 `get_announcements_deduped()`에서는 id=10398 하나로 병합 노출됨. Drive 원문에서 두 블록의 주소(마송5=B-5BL=경기도 김포시 통진읍 율마로438번길 51, 마송6=B-3BL=경기도 김포시 통진읍 율마로450번길 51)와 사진·평면도 파일은 블록별로 명확히 구분되어 있으나, **MYHOME sub-row의 277/199 세대수 중 어느 쪽이 B-3BL·B-5BL 중 어느 블록에 해당하는지는 공고문 텍스트만으로 확정하지 못함**(임의 추정 안 함) — 이번 세션엔 `relaxation_detail`/`selection_method`/`contract_before_verification`만 병합된 대표행(id 10398) 등 6개 행 전체에 공통 백필했고, `precise_address`와 `announcement_extras`(사진 25건, B-3BL/B-5BL 각각 라벨링됨)는 블록별 오매칭 위험 때문에 보류함. 다운님이 어느 announcement_id가 몇 세대(277 vs 199)인지 원문/스프레드시트로 확인해주시면 블록별 precise_address·extras 반영 가능
