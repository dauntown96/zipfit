...
| 날짜 | 내용 |
|---|---|
| 2026-09-25 | **B32 — 작은 정정 reverify(`…020819` 비치 품목 · 7/6 군산 `20722_1` 금액 칸 · 양산대석 `…020817` 정정) · `…020773` 상태 · `…020202` 목록 밖 사진 2행**(🔴 **정책 INSERT 1 · UPDATE 1 · DELETE 1(`…020819`) · 세대 UPDATE 2행(`20722_1` — 월임대료·`rent_priority1`·`source_file`) · `announcement_analysis` UPDATE 1(`…020773` → 완료) · INSERT 1(`…020817` 완료 · B32) · extras UPDATE 1(`c9ddbfff…` 「고은」 `supply_list`) · DELETE 2(백업 PR #218 → `43bfb06` 병합 뒤) · DDL 0 · 함수 0 · 코드 0 · EF `fetch-attachment` 3회(`[임시] 2015122300020817` 폴더 1 · 정정본 PDF·hwpx 2) · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 reverify v4.7 · analysis v9.0. ① ✅ **`…020819`** — ⚠️ 비치 품목 행은 모순이 아니다(두 문장 모두 「등」 예시) → p.1 머리말 글머리를 신규 행 「가전제품 등 비치」로(`40af099c…`) · p.11 문장은 hwpx 표 행 8 「기타사항」 셀의 첫 글머리인데 기존 행이 빠뜨려 앞에 더함(`fe8faadb…` → `eebf3c03…`) · ⚠️ 행 삭제(`17a270f2…`) · 29 = 29. ② ✅ **`20722_1`** — 두 행 모두 월임대료 칸 2개에 같은 조의 계약금이 들어 있었다(167,900→74,730 · 391,400→116,560 · 120,200→47,860 ×2, 7/6 원문 p.2) · `source_file` 실재하지 않는 「…(20260721).pdf」 → 실파일명 · 나머지 칸 불변(jsonb 대조) · `…020822` 1층 요약 `rent_max` 391,400 → 116,560. ③ ✅ **양산대석** — 정정 diff(PDF 줄 단위): 1~32쪽 동일 · 차이는 본인 행정정보 제공 요구서 양식(옛 p.33~39 → 새 p.33~35)뿐 · 닿는 DB 값 0 → 산물은 `…020754`에 두고 분석 행만 `…020817`에(칠곡 `…020810` 선례). ④ ✅ **`…020773` 남은 3장** — 셋 다 6/30에 `…020202`에 이미 붙은 파일과 바이트 동일(새 행 0) · 월곡동만 6/25 청년 목록(49행)에 있어 기존 행에 키 · 연향동 원도빌 · 가곡동 성하파밀리아는 청년·신혼Ⅰ(37)·Ⅱ(22) 목록 어디에도 없어 백업 → 삭제(1,861 → 1,859). 불변식 1·6·7·9(대상) 0 · 10(전 테이블) 0 |
| 2026-09-25 | **B31 — 9월 회차 이미지 연결(광주전남 4 · 원주 — `unit_key` 방식 첫 회차) · 따로 선 취소공고 카드 표시**(🔴 **DB INSERT 55행(`announcement_extras`, 공고마다 한 트랜잭션·가드 6) · `announcement_analysis` UPDATE 5행(상태·`pending_fields[1]`) · DDL 0 · 함수 0 · EF 0 · 코드 변경 `index.html`(`zfIsCancelRow`·`cancelRowChip`·`cancelRowDateLine` 신설, 공고 탭 카드·정렬·추천·매칭) + `sw.js` `CACHE_NAME` v141 → **v142** · Drive 쓰기 0 · Notion ⑩ 1절** · `[zipfit-backup]` 접촉 0). ① ✅ **연결** — 스킬 v9.0 · 폴더는 `drive_folder_id`로 열어 다운님 업로드분(9/23 14:40~15:27Z)만 대상 · 광주전남은 공급주택목록 PDF의 주소 열을 매개로(파일명 → 목록 주소 → `housing_units` 주소 → 건물 머리 키), 원주는 공고문 모집표(무실7 51 · 흥업2 36 · 태장5 26B)로. `…020773` 23(26 중 3건은 이번 목록에 없는 주소 — 미연결, 붙이지 않음) · `…020777` 17(18 중 삼학로227번길 19는 17과 바이트 동일 — 한 장만) · `…020784` 4 · `…020819` 2 · 원주 `21271_1/2/3` 9(평면도 3 키 · 배치도·위치도·조감도 6 NULL). `unit_key` 채운 행 49 = `image_link` 49 · 불변식 9·10(대상 ID) 0 · 다른 공고 extras `id‖file_url` `5723b1c0…` 불변 · 1,806 → **1,861**. 상태: 0777·0784·0819·0783 → 완료 · 0773 완료(보조 누락) 유지. ② ✅ **화면 대역**(봉인 hu `9c9beae0…`·x `a2cb388c…` = DB · 360px): `…020773` 건물 머리 칩 23개 합 23 · 원주 블록 `21271_1/2/3` 주택형 머리 칩 각 1 + 갤러리 2·4. ③ ✅ **취소공고 카드**(RPC 봉인 `402da684…` = DB): 태백 `…020645`·청주 `…020011` — 「🚫 취소공고」 칩만 · 상태 칩·접수기간 칩·원문 정리 전 상자 없음 · 「모집공고: … · 📄 취소공고 원문 보기」 · 세 정렬 모두 맨 뒤(종전 「최신 공고순」은 접수 중 공고 앞, 「마감 임박순」은 맨 앞) · 매칭 결과에서 빠짐 · 거울 경고 0 · `pageerror` 0. ④ **미확인**: 🔴 배포본 육안 — claude.ai Playwright 몫 · 추천 탭 제외는 코드 경로로만 |
| 2026-09-25 | **B30 — 취소공고 배지·취소행 짝짓기에 회차 조건(제목 키 + 접수 마감일)**(🔴 **함수 1(`get_announcements_deduped` `CREATE OR REPLACE` — 마이그레이션 `b30_cancel_keys_round` · 반환 타입·ACL 불변) · DDL 0 · 데이터 쓰기 0 · EF 0 · 코드 변경 `index.html`(`zfCancelEndMatch`·`zfCancelInfoFor` 신설, `zfPairedCancelIds`·`zfCheckDedupMirror`·`zfResolveSavedCards`·`cancelNoticeBadge` 호출 4곳이 `apply_end`를 함께 본다) + `sw.js` `CACHE_NAME` v140 → **v141** + `supabase/rpc/` 덤프·README · Drive 0 · Notion ⑩ 1절** · `[zipfit-backup]` 접촉 0). ① ✅ **덤프 먼저** — 저장소 = DB `819f1443…`(원칙 20) → 새 정의 `72da7fa5…`. `cancel_keys`는 `title_key` + `apply_end` · 배지는 제목 키가 같고 마감일이 같을 때만(한쪽 NULL이면 종전처럼 제목만) · `title_key`라 P1으로 떼어진 회차도 짝이 된다. ② ✅ **전·후 한 조회**(10:07Z, 옛 정의 ↔ `pg_temp` 새 정의): 906 = 906 · ID 집합 차 0 · 다른 칸 전부 같음 · 배지 참 2 → 0(태백 `21199_1` · 청주 `…020066` — 둘 다 다른 회차에 붙어 있었다). ③ ✅ **화면 대역**(RPC 응답까지만 대역 · 배포 코드 `zfMapNoticeRows` 경유 · 봉인 새 `402da684…`·옛 `4978a59e…` = DB · 360px): 태백 `21199_1` 배지 없음 · 취소행 `…020645` 목록에 남음(제목 「[취소공고]태백철암1…」 · 「접수마감」 칩 · 「📅 접수기간 미상」 · 모집공고 2026-08-31 · 접수 미상 ~ 2026-10-14) · 청주 `…020066` 배지 없음 · `…020011`도 남음(마감 숨김을 끄면 보인다) · 거울 경고 0 · `pageerror` 0. 🔵 **엇갈린 배포 대역** — 새 화면 + 옛 DB는 배지(링크 없음)와 취소행이 함께 보이지만 옛 화면 + 새 DB는 둘 다 사라진다 → **화면 병합을 먼저, 함수 적용을 뒤에** 했다. ④ **미확인**: 🔴 배포본 육안 — claude.ai Playwright 몫 |
