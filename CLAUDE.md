...
| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **B23 — 원주 국민임대 [정정] `…020783`과 군산나운4 영구임대 입주자격완화 `…020801`(접수 9/29)을 원문으로 분석해 반영했다. 둘 다 `완료(보조 누락)`**(🔴 **DB 쓰기: 원주 — `housing_units` INSERT 3행(블록 `21271_1/2/3` 한 행씩) · `eligibility_criteria` 3행(블록별) · `announcement_policies` 26행(LH `…020783`) · `announcement_analysis` 1행 / 군산 — `housing_units` 2행 · `eligibility_criteria` 1행 · `announcement_policies` 21행 · `announcement_analysis` 1행(모두 `…020801`) · `announcements.relaxation_detail` UPDATE 1행(`…020801`, NULL이던 칸) · 그 밖의 UPDATE·DELETE 0 · DDL 0 · RPC·트리거 0 · EF 배포 0 · 코드 변경 0(`index.html`·`sw.js` 미변경 → `CACHE_NAME` 그대로 **v137**) + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 10파일(LH → Drive, `[임시]` 폴더 5개: 원주 정정 3벌 · 군산 두 회차) · Notion 쓰기 0** · `[zipfit-backup]` 접촉 0). ① ✅ **운반 관문** — 10파일 전부 6MB 미만(최대 1,146,080B). 첨부는 공고문 hwpx·PDF뿐이다. ② ✅ **정정 diff**(PDF 텍스트 줄 단위, 3벌 전부) — `…020677`→`…020767`은 원주무실7 51형 세대당 계약면적 한 줄(주거전용 36.62→51.93 외 4칸), `…020767`→`…020783`은 원주태장5 26B 최대전환 감액 월임대료 한 칸(113,310→133,310)이다. 정정 전 값은 감액 3.5% 검산에 걸리고 정정 후 값은 통과한다. ③ ✅ **규약 29** — 원주는 블록 6(지난 회차 `20759_*` 3 포함)이라 이번 회차 블록 `21271_1/2/3`에 단지별로 넣고, 공통 조항은 LH 대표 `…020783`에 넣었다. 군산은 블록 1이다. ④ ✅ **전환 2단 관문** — 원주 3행 모두 증액 6.0%·감액 3.5% 일치. 군산은 전환 열이 없어 「재료 없음」(`extra_note`에 ⚠️). ⑤ ✅ **완화** — 군산 `is_relaxed_recruitment` true · `contract_before_verification` false 일치(불변식 5). 소득·총자산 전면 배제라 `income_limit_exempt`·`asset_limit_exempt` true · `income_pct` NULL. ⑥ ✅ **⚠️ 행** — 원주 서류제출 방법(p.12·p.13 「인터넷·등기우편」 ↔ p.16 「방문 또는 등기우편」), 군산 완화 입주자 재계약(p.1 ↔ p.12 ↔ p.34 별지7 — 지난 회차 7/6과 같은 구조). 둘 다 자격 관문이 아니라 판정 대기로 올리지 않았다. ⑦ ✅ **불변식** — 공고마다 한 트랜잭션에서 hu·정책 canonical md5와 1~7·9를 확인했다. 두 그룹 형제 스냅샷(20 ID) md5 `a95a6ec7…` 전·후 불변 · 불변식 v3.2 위반 0. ⑧ 🔵 **🅑 분석률(개정 산식, 대표와 같은 `apply_end`)** — 접수 전 **76/39 → 76/40**(14:50:17Z → 15:04:48Z). +1은 원주다. 군산은 `get_announcements_deduped()` 대표가 새 회차 `…020822`(공고 9/23·접수 10/6, 406동 A·B형)라 `…020801`이 대표와 같은 회차가 아니어서 분자에 들어가지 않는다. ⑨ **미확인**: 없음 — 코드 변경이 없다 |
| 2026-09-23 | **B22 — 기본기준 매입임대/전세형 `car_limit`을 원문대로 비우고(불변식 v3.2가 그 한 행만 예외), 근거 공고 없는 기본기준 행을 금액과 무관하게 📄로 보내고, 행복주택/청년 가산을 원문 표의 가구원 수별 최대로 자르고, ✅가 빈 매칭에 안내 한 줄을 달고, 🎂·💍·📄 라벨에 「(일반)」 규칙을 썼다. 재분석 큐는 조사만 했다**(🔴 **DB 쓰기 `eligibility_criteria` UPDATE 2행(전세형 `39d44cc1` `car_limit` 36,200,000→NULL · `basis_note` 한 단락 / 행복주택/청년 `420b86ad` `verification_requirements['가구원수별_가산상한']` 신설 · `basis_note` 한 단락) · DDL 0 · RPC·트리거 0 · EF 배포 0 · 코드 변경 `index.html`(기본기준 조회에 `basis_announcement_id` · `noBasis`·`ZF_MSG_NO_BASIS` · `shownForCheck` 근거 없는 행 통과 · 📄 머리말 근거 없는 행 전용 문구 · `limitsFor` 가구원수별 상한 · 🎂·💍·📄 `sfLabelOf` · `matchNoTypeFilter` 안내) + `sw.js` `CACHE_NAME` v136 → **v137** + `supabase/invariants/v3.2.sql` 신설 · README + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 0 · Notion 쓰기 ⑩** · `[zipfit-backup]` 접촉 0). ① ✅ **전세형 자동차** — 나머지 칸 md5 `6e3fc7fd…` · 그 밖 기본기준 9행 `1b1a8958…` **불변** · 11행 불변. 불변식 v3.2 위반 **0**. 되돌리는 트랜잭션에서 옛 V7은 전세형을 잡고, v3.2는 국민임대 `car_limit` NULL과 전세형 `asset_limit` NULL을 여전히 잡는다. 🔴 v3.1·v3 파일은 `v7`·`v8` 사이 쉼표가 없어 그대로는 42601로 죽는다(v3.2에서 넣음, 옛 파일 미수정). ② ✅ **봉인 REST 응답** 구 `1fa40429…`(= B20) · 신 `7600ef87…`(DB md5 일치, 구 응답에서 변환해 만듦)으로 `diagnose()` 전·후 — 기혼 3인·400·1자녀·아니오·3.5억·차 4,000만 → 구 ✅ 없음 · 신 ✅ 매입임대(전세형) · 자동차 「판정에서 뺐어요」 범위 3,620만원~4,542만원 → 4,542만원 · 5.5억 → 분양이 📄에 남음(구: 사라짐) · 📄에 분양만 남는 입력(3인·900·5.5억)에서 머리말이 「통과한 유형이 남아 있지만」 대신 근거 없는 행 문구 · 행복주택/청년 표 안 여섯 조합 소득 경계(457/458 · 645/646 · 703/704 · 816/817 · 898/899 · 980/981) **전·후 동일** · 표 밖 1인·1자녀 458만·2인·2자녀 704만 → 구 통과 · 신 탈락(120%) · 자산 1인·1자녀 2.60억·2인·2자녀 2.90억 · 자동차 1인·1자녀 4,800만 → 구 통과 · 신 탈락 · 표 안 자산 조합 불변 · 합성 응답으로 🎂 「행복주택(일반)」 · 💍·📄 「매입임대(일반)」 · 매칭 합성 60행 → ✅ 빈 경우만 안내 한 줄 · 건수 45 = 45 · `pageerror` 0. ③ 🔵 **재분석 큐 조사만** — `get_reanalysis_queue()`(저장소 덤프 = DB md5 `bb1c4032…`)는 그룹을 `announcement_dedup_key(title)`로 묶고 자식이 있는 구성원을 전부 donor로 올린다. 회차 조건이 없다. 큐 13건 = 같은 회차 donor만 7 · 섞임 1 · 지난 회차만 5. ④ **미확인**: 🔴 배포본 육안 — claude.ai Playwright 몫 |
