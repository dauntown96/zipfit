...
| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **B16 — 신혼·신생아·청년·다자녀 매입임대 10공고의 재계약·할증 정책 행을 원문으로 바꾸고 옛 요약 행을 걷었다. Ⅰ 다섯 공고에 공고문 「소득·자산 기준」 원문 행을 넣었다. 마감 공고 3행을 고쳤고, `…020673` 세대 43행을 LH ID로 옮겼다. ⑩에 「공고 카드는 정책 행을 어느 ID 범위에서 읽는가」를 더했다**(🔴 **DB 쓰기 `announcement_policies` 신규 10행(멈춤 전 5 · 확인서 뒤 5) · UPDATE — 멈춤 전 각주 4행(750·795·776·791) · 교체 12행(항목 1 9 + 항목 5 3) · `source_file`만 2행 · category 2행(교체 행과 겹침) · DELETE 18행(1-5 15 · 1-6 2 · 2-2 1) · 총수 1,335 → 1,340(멈춤 전) → **1,327** · `housing_units` UPDATE 43행(`announcement_id` → LH `…020673`) · DDL 0 · RPC·트리거 0 · EF 배포 0 · 코드 변경 0(`index.html`·`sw.js` 미변경 → `CACHE_NAME` 그대로 **v133**) + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 0 · Notion 쓰기 ⑩ 1곳** · `[zipfit]` **PR #201** · `[zipfit-backup]` 접촉 0). ① ✅ **교체** — 첨부 AFTER(QnA·공고문 원문 문장)대로 9행이다. 188 `fc8be681`·239 `c17e46b1`에서는 확인서 판정 C에 따라 「입주 후 소득수준의 향상으로 소득 70%(…)를 입주자가 계속 거주하기를 …」 한 줄을 뺐다. 원문 PDF가 「초과한」 자리를 공란으로 렌더해, 그 줄을 그대로 실으면 뜻이 바뀐다. 같은 사실은 공고문 행 `e76b5809`·`2d428516`에 이미 있다. 이혼 줄은 공란(「신혼」)이 있어도 뜻이 유지돼 원문 그대로 두었다. ② ✅ **삭제 18** — 행마다 사실을 담은 교체·신규 뒤 행 id를 `position()`과 md5로 확인하고 지웠다(빈칸 0 · 건너뜀 0). 의존 교체 03:00:09Z → 삭제 03:01:07Z. 1-6은 239 `5e1461ad`·255 `ac61342c`만 지웠다(근거 `c17e46b1`·`06eee738`). 195 `b34668d2`·196 `d9a6587e`(예비신혼부부 혼인신고)는 같은 공고의 원문 행에 그 사실이 없어 **남겼다**. ③ ✅ **항목 2** — 188·195·208·239 공고문 PDF 네 건의 「소득ž자산 기준」 블록이 글자까지 같고, 675 hwpx 셀 구조와도 같다. 776 `e7dc6246` 꼴로 평탄화한 한 문안(md5 `2e714ce6…`)을 다섯 공고에 넣었다. PDF의 칸 끼어듦은 위치 순서로 확인했다. 675 요약 행 `1f8ff635`는 사실 7개가 전부 새 행에 있어 지웠다. ④ ✅ **항목 5** — 200 `95b95512`은 공고문 배점표 원문으로 바꾸고 `source_file`을 실파일명으로 고쳤다. 313 `2db4da64`은 메모만 걷었다. 315 `5fd2550d`은 다른 정책 행이 없어 원문 두 문장으로 교체하고 category를 「임대차계약」으로 바꿨다. ⑤ **검증** — 26행 중 남은 11행과 이번 신규 10행의 일반명 `source_file` **0** · 대상 18공고 그룹 형제 정책 행 **0** · 673 LH 43 / MYHOME 0·0 · 🅑 **76/41 · 7/1 → 76/41 · 7/1**(02:30:42Z → 03:07:24Z, 분자 불변). ⑥ **하지 않은 것**: 26행 밖 일반명 `source_file` 27행 **0** · 673 요약 9행 **0** · 활성 9공고의 옛 회차 정책 44행 **0**(다음 재확인 묶음) · `[zipfit-backup]` **0**. ⑦ **미확인**: 없음 — 코드 변경이 없어 배포본에서 볼 것이 없다 |
