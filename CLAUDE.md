...
| 2026-09-26 | **B44 — 9/29 접수 수도권 정정공고 2건 분석·반영(강서염창 통합공공임대 최초 입주자 `2015122300020807` · 화성동탄2 10년 분양전환(리츠) 예비입주자 `0000061177` 블록 8) + 항목 3 조사(읽기만)**(🔴 **세대 INSERT 32(6·26) · 자격 INSERT 10(2·8) · 정책 INSERT 108(70·38) · `announcement_analysis` INSERT 2(둘 다 `완료(보조 누락)`) · `announcements` UPDATE 1(`0000061177` `apply_start` 2026-09-29 — 원문 「청약 및 계약 등 주요 일정」 표) · DELETE 0 · DDL 0 · 함수 0 · EF 0 · 코드 0(`index.html`·`sw.js` 불변) · Drive 0(업로드는 착수 전 12건 완료분) · Notion 0** · `[zipfit-backup]` 접촉 0). 스킬 analysis v9.1 · reverify v4.8을 착수 전에, analysis를 압축 뒤 첫 DB 쓰기 전에 다시 불러왔다. ① ✅ **정정 diff 먼저** — 강서: 우선공급 「(현장접수만 가능)」 · 일반공급 현장접수 문단 신설 · 입주안내 연도 ‘26→‘27 · 끝 날짜 9.17→9.21 / 동탄: 「연원일」→「연월일」 · Ⅶ-1 계약포기 문장 · 「당첨자(예비입주자)는」 삭제 3곳(Ⅶ-2의 「정당한 사유 없이 … (당첨자로 관리)」는 정정본에도 남음). 반영은 정정본 기준. ② ✅ **강서 `…0807`**(블록 1 → 대표 ID에 전부) — 59A 14호를 소득구간 1~6구간 6행(`unit_group` 59A · `group_recruit_count` 14 · `supply_target` 구간)으로, 증액 역산 6.0% 6/6 · 감액 3.5% 6/6(10원 절사) · 계약금 5% · 자격 2행(일반공급 150%+우대 · 우선공급(철거민 등) 소득·자산 미적용) · 정책 70(17·18장 법령 별표는 제외) · 할증 조항의 한컴 사용자 영역 번호 기호(U+F02B1~3)는 원문 코드값 그대로. ③ ✅ **동탄 `0000061177`**(블록 8 → 세대·자격은 `21296_2`~`_9`, 정책·분석은 대표 ID) — 26행 · 블록 합 21·15·31·22·27·10·24·44 = 194(원문 합계 · 면적대 합 · MYHOME 블록값 3중 일치) · 원문 요율 「증액 5% / 감액 3.5%」로 26/26 검산(0장 판정 첫 적용) · 계약금 20% · 정책 38. ④ ✅ **항목 3** — `selection_method`는 `buildConvBadges` 3값 `===` 비교뿐(모르는 값·NULL은 배지 없음) · 매입 `unit_type` 문자열 분기 0(키·라벨로만) · `area_max`·`total_units`는 정렬·상세표·블록 줄 대체값. ⑤ ✅ **검증** — 공고마다 블록 트랜잭션 3개(블록마다 md5 봉합 · 마지막 블록에 불변식 1·2·3·5·6·7·9·NFC) · 자격 JSON md5 DB = 로컬 · 형제 산물 0(강서 3 · 동탄 17) · 🅑 접수 전 **76/45 → 76/47**(06:57:40Z → 07:17:34Z · 분자 +2 = 닫은 공고 2) · 접수 중 7/1 불변 |
