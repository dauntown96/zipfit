...
> **마지막 업데이트**: 2026-07-03 (announcement_extras orphaned id 5개 재매핑 완료 — DB만, 프론트엔드 노출 코드 자체가 없음을 확인함)

## 🔜 다음 세션 작업 예정
- **🟠 announcement_extras(사진·평면도) 프론트엔드 노출 코드 신규 구현 필요**: DB 매핑(355건 전부 매칭 완료, orphaned id 0건)은 끝났지만 index.html에 이 테이블을 읽는 코드가 애초에 존재하지 않음(`extra_type`/`file_url`/`dong_ho`/`building_address` 전부 미검색). housing_units/announcement_policies와 동일한 패턴(카드 상세 오픈 시 announcement_id로 fetch)으로 신규 구현 필요
