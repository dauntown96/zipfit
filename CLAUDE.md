...
| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **B17 — 설정 탭 조건 폼이 토큰 갱신·탭 재진입 때 저장값으로 되돌아가던 것을 막고(세션당 1회 채움), 정책 칸 mixed 배너를 화면에 없는 「행마다 회차」 표시를 가리키지 않는 문구로 바꿨다. 주택 매칭·서비스 워커·전세형 자산 가산 NULL은 조사만 했다**(🔴 **DB 쓰기 0 · DDL 0 · RPC·트리거 0 · EF 배포 0 · 코드 변경 `index.html`(`zfSettingsFormApplied` 플래그 · `applySettingsToUI` 조건 폼·설정 칩 감싸기 · 로그아웃 두 곳 리셋 · `STALE_SRC_MSG_MIXED_POLICY` 신설 · `staleSourceBannerHtml` 7번째 인자 `mixedMsg` · 정책 로더 호출부) + `sw.js` `CACHE_NAME` v133 → **v134** + `CLAUDE.md` 이력 회전 + `docs/history.md` 1줄 · Drive 쓰기 0 · Notion 쓰기 ⑩** · `[zipfit-backup]` 접촉 0). ① ✅ **토큰 갱신 재현 → 수정** — 가짜 세션·가짜 프로필 대역(자격증명 없음)으로 배포 코드를 헤드리스 크롬에서 돌렸다. 저장 버튼을 누르기 전 `TOKEN_REFRESHED` 1회에 고치던 값(혼인·소득·자녀·유형 칩)이 **전부 저장값으로 되돌아갔고**, 설정 탭을 다시 여는 경로(`goMain(5)` → `renderSettingsGate`)도 같았다. 수정 뒤 두 경로 모두 **입력 유지** · 첫 조회 실패 뒤 성공·로그아웃 뒤 다른 프로필 로그인·저장 뒤 갱신 세 시나리오는 **전·후 동일** · 추천 탭 PR #166 칩 동작 불변 · `pageerror` 0. 🔴 플래그는 프로필 조회가 **성공한 뒤에만** 올린다. ② ✅ **정책 칸 문구** — 「여러 회차 공고의 안내가 함께 표시되고 있습니다. 이번 공고와 다른 내용이 있을 수 있으니 공고 원문과 함께 확인하세요.」 정책 로더를 그대로 돌려(649 그룹 실물 ID·회차 날짜, 정책 본문은 합성) 새 문구를 확인했고, 세대별 정보 칸의 mixed·split·past-only 문구는 **불변**이다. ③ 🔵 **조사만** — 매칭은 `window.eligibleTypes`(= ✅ 상자의 `housing_type` 집합)를 `housing_type` 부분 문자열로 거른다(공급형태 축 없음 → 신혼Ⅰ 통과면 청년 매입임대도 뜬다). 소득 구간 select는 **아무 데서도 읽히지 않는다**(읽는 `setSort()`는 호출처 0). 정렬은 접수 단계 → 회차 날짜뿐이라 미분석 공고가 위로 온다. 서비스 워커는 이미 network-first + `skipWaiting` + `clients.claim`이고, 로컬 재현(GitHub Pages와 같은 `max-age=600`)에서 **HTML은 첫 재방문부터 새 것**이었고 `CACHE_NAME`만 몇 초 늦게 바뀌었다. 전세형 자산 가산 NULL은 `limitsFor`의 `pick()`이 기본 한도(3.62억)로 떨어져, 자녀 1·자산 3.70억이면 매입임대(전세형)가 **어느 상자에도 안 나온다**(REST 응답 세 개 md5 봉합). ④ **하지 않은 것**: 항목 1·2·4 수정 **0** · DB **0** · `[zipfit-backup]` **0**. ⑤ **미확인**: 🔴 배포본 육안(설정 탭 입력 유지 · 정책 칸 새 문구) — 프록시 403이라 claude.ai Playwright 몫 |
