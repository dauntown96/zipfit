...
| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **B24 — 재분석 큐에 「같은 회차 완료분 제외」 한 줄 · 자격진단 총자산 안내 한 줄 · 👫 라벨 「(일반)」 · 불변식 README 한 줄**(🔴 **DB 함수 `get_reanalysis_queue()` `CREATE OR REPLACE` 1회(마이그레이션 `b24_reanalysis_queue_same_round_done`, md5 `bb1c4032…` → `fa40c50b…` = 저장소 덤프, ACL 불변) · 테이블 쓰기 0 · DDL 0 · EF 배포 0 · 코드 변경 `index.html`(총자산 입력 아래 안내 `<p>` · `dualLabels`가 `sfLabelOf`) + `sw.js` `CACHE_NAME` v137 → **v138** + `supabase/rpc/get_reanalysis_queue.sql`·README + `supabase/invariants/README.md` 1줄 + `CLAUDE.md` 이력 회전 + `docs/history.md` 2줄 · Drive 쓰기 0 · Notion 쓰기 ⑩** · `[zipfit-backup]` 접촉 0). ① ✅ **큐** — 대표와 같은 `apply_end`의 그룹 구성원이 완료 계열 `announcement_analysis`를 가지면 뺀다(⑨ 5장 분석률과 같은 축, 대표 `apply_end` NULL이면 종전대로). 적용 전 12건(16:03Z, A 같은 회차만 7 · B 섞임 1 · C 지난 회차만 4) → 적용 후 **4건**(16:04Z, C 4건 `…0687`·`…0690`·`…0694`·`…0822` 그대로). A 7과 B 1(양산대석 `…0817`)이 빠졌다 — B22 회신 예측과 같다. B25 조사 1의 권고(P1 — 열린 LH 회차만 떼어 냄)는 그룹 키를 넓게 바꾸지 않아 멈춤 ① 대상이 아니었다. ② ✅ **총자산 안내** — 「부동산·자동차·금융자산 등을 모두 더하고 부채를 뺀 금액이에요.」(공고 공통 정의 `…020255` p.5를 옮김). 판정 코드는 그대로. ③ ✅ **👫** — `dualLabels`도 `sfLabelOf(c)`. ④ ✅ **검증**(배포 코드 전·후 두 벌 · 봉인 REST 응답 B20 `1fa40429…` · B22 `7600ef87…` · 입력 13개 — 자산 1.5억~5.5억 경계 8 · 미혼1인 1 · 맞벌이 모름 4): 통과 유형·박스 **전·후 동일**이고 달라진 것은 👫 줄 「매입임대」 → 「매입임대(일반)」 2건뿐 · 안내 문구는 모바일 390px 렌더에서 입력 아래 한 줄 · `pageerror` 0. ⑤ **미확인**: 🔴 배포본 육안 — claude.ai Playwright 몫 |
| 2026-09-23 | **B25 — 조사 3건(회차 축 · 이미지 불러오기 실패 296 · 사진·평면도를 세대 행 안으로). 관측만 했다**(🔴 **DB 쓰기 0(SELECT와 `net.http_get` 관측 요청만) · DDL 0 · 함수 0 · EF 0 · 코드 변경 0 · Drive 쓰기 0 · Notion 쓰기 ⑩ 3절 추가** · `[zipfit-backup]` 접촉 0). ① 🔵 **회차 축** — `deduped`·`group_ids`·`blocks`·`queue`는 전부 `announcement_dedup_key(title)` 한 키이고 회차 조건이 없다(`price_summary`만 회차 날짜). 그래서 **군산 `…020801`(접수 9/29)이 같은 제목의 새 회차 `…020822`에 가려 목록에서 빠져 있다**. 대표와 회차 날짜·`apply_end`가 둘 다 다르고 LH `공고중`·`접수중`인 회차는 지금 이 1건(15:57Z). 최소 처방 P1(그 회차만 `dedup_key`에 꼬리) · P2(상속 회차 키 전면, +107장) · P3(회차 식별자 컬럼)을 회신했다. ⓑ 다른 회차 블록이 섞인 카드는 원주 `…0783` 하나(6블록 중 3) · ⓒ 과잉 분리는 접수 전·중 4 panId → 8장. ② 🔵 **이미지 296** — 1,920개 전수(15:48Z): 로그인 페이지 207(9/12 「207개 없음」과 같은 집합, 9/13에는 익명으로 보였다가 다시 막힘 · 7월 수동 업로드분) · 썸네일 404 89(파일은 있고 익명 `/preview`는 200 — 87개가 9/22 수동 업로드 매입임대 PDF). 표본 실패 3 · 정상 3을 비교했는데 파일 쪽 차이는 없었다. ③ 🔵 **세대 행 배치** — 두 구역을 잇는 키가 없다. 매입임대 1,040행 중 캡션을 건물명 또는 주소와 맞추면 830(80%) 적중 · 210행은 문자열로 못 잇는다. ④ **미확인**: 207개가 삭제인지 공유 해제인지 · 썸네일 미생성 원인 |
